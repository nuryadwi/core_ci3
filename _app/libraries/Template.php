...
class Template {
	...
	public function get( $position, $string = null ) {
    ...
    public function customize_params( $cos = [] ){
		foreach ( $cos as $key => $val ) {
			$this->template_params[$key] = $val;
		}
	}
	...
	public function show( $template_view = 'template', $return_string = true ) {
		if ( isset( $this->user_info ) ) {
			if ( ! isset( $this->user_menu ) || empty( $this->user_menu ) ) {
				if ( $this->user_info['user_group_name'] == 'root' ) {
					...
					$params = [
						'table' => [
							...
							'core_menu' => 'user_group_role_menu_id = menu_id'
						],
						'select' => 'menu_id, menu_parent_menu_id, menu_name, menu_slug, menu_url, menu_description, menu_image, menu_class, menu_sort',
						'where' => [
							'menu_is_active' => '1',
							'user_group_role_user_group_id' => $this->user_info['user_group_id']
						 ]
					];
					$query_menu = get_data( $params );
				}
