...
class Backend_Controller extends MY_Controller
{
   public function __construct() {
      ...
      if (!$this->auth->auth_user()) {
			...
			$redirect = 'login?redirect_url=' . $referer;
			redirect($redirect);
		} else if (!$this->auth->privilege_user()) {
         ...
         $query_menu = get_data(
            [
               ...
               'table'  => [
                  'core_menu child' =>'',
                  'core_menu parent' => [
                     'child.menu_parent_menu_id = parent.menu_id','left'
                  ]
               ],
               'where' => [
                  'child.menu_url' => uri_string(),
               ]
            ]
         );
   ...
   function get_breadcrumb() {
      $breadcrumb = [];
		if (!empty($this->info_menu)) {
			if (!empty($this->info_menu->parent_name)) {
				$breadcrumb = [
					$this->info_menu->parent_name => (($this->info_menu->parent_url == '#') ? $this->info_menu->menu_url . '#!' : $this->info_menu->parent_url),
